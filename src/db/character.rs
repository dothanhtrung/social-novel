...
pub async fn get_all(pool: &PgPool) -> Result<Vec<Character>, Error> {
    sqlx::query_as!(Character, r#"SELECT * FROM character ORDER BY username DESC"#).fetch_all(pool).await
}
...
pub async fn get(pool: &PgPool, username: &str) -> Result<Character, Error> {
    sqlx::query_as!(Character, r#"SELECT * FROM character WHERE username = $1"#, username).fetch_one(pool).await
}

pub async fn delete(pool: &PgPool, username: &str) -> Result<(), Error> {
    sqlx::query!(r#"DELETE FROM character WHERE username = $1"#, username).execute(pool).await.map(|_| ())
}